...
class AGEMPlugin(SupervisedPlugin):
    ...
    def before_training_iteration(self, strategy, **kwargs):
        ...
        if len(self.buffers) > 0:
            ...
            for xx in xref:
                new_xref += xx
            xref = tuple(_.to(strategy.device) for _ in list(new_xref))

            new_yref = ()
            for yy in yref:
                new_yref += yy
            yref = tuple({k: v.to(strategy.device) for k, v in yy.items()} for yy in list(new_yref))

            detection_loss_dict = strategy.model(xref, yref)
    ...
    def update_memory(self, dataset, num_workers=0, **kwargs):
        ...
        self.buffer_dataloader = GroupBalancedInfiniteDataLoader(
            ...
            collate_mbatches=detection_collate_fn
        )
