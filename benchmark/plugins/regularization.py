        ...
        if self.prev_model is None:
            ...
            if isinstance(self.prev_model, MultiTaskModule):
                ...
                with torch.no_grad():
                    ...
                    y_prev_lst = []
                    for xx in x:
                        yy = self.prev_model.backbone(xx.unsqueeze(0))
                        yy = next(reversed(yy.values()))

                        y_prev_lst.append(yy)
                    y_prev = {0: torch.cat(y_prev_lst, 0)}
                    self.prev_model.train()

                curr_model.eval()
                y_curr_lst = []
                for xx in x:
                    yy = curr_model.backbone(xx.unsqueeze(0))
                    yy = next(reversed(yy.values()))

                    y_curr_lst.append(yy)
                y_curr = {0: torch.cat(y_curr_lst, 0)}
                curr_model.train()
