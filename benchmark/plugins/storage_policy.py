...
class ExperienceBalancedBuffer(BalancedExemplarsBuffer):
    ...
    def update(self, strategy: "SupervisedTemplate", **kwargs):
        ...
        for ll, b in zip(lens, self.buffer_groups.values()):
            buffer_data = list(b.buffer)[:ll]

            clean_dataset = SimpleDataset(buffer_data)
            cls_dataset = make_classification_dataset(
                clean_dataset,
                transform_groups=None,
                initial_transform_group="train",
                collate_fn=detection_collate_fn
            )
            cls_dataset = classification_subset(cls_dataset, indices=list(range(len(cls_dataset))))

            new_b = ReservoirSamplingBuffer(ll)
            new_b.update_from_dataset(cls_dataset)

            b.buffer = new_b.buffer
...
class FeatureBasedExemplarsSelectionStrategy(ExemplarsSelectionStrategy, ABC):
    ...
    def make_sorted_indices(
        ...
        features = []
        for idx, (x, y_true, _) in tqdm(enumerate(DataLoader(
                data,
                collate_fn=collate_fn,
                batch_size=strategy.eval_mb_size,
        ))):
            x = tuple(_.to(strategy.device) for _ in list(x))
            fea = self.feature_extractor(x)
            fea = fea.to(strategy.device)

            features.append(final_pool(fea))
