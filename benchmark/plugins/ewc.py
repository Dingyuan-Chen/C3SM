...
class EWCPlugin(SupervisedPlugin):
            ...
            for experience in range(exp_counter):
                for k, cur_param in strategy.model.named_parameters():
                    ...
                    if not self.is_incdet:
                        ...
                        cur = cur_param
                        saved = saved_param.expand(new_shape)
                        imp_masked = imp.expand(new_shape)

                        abs_diff = (imp_masked ** 0.5) * (cur - saved).abs()

                        quad_part = imp_masked * (cur - saved).pow(2)
                        linear_part = (2 * beta * (imp_masked ** 0.5)) * (cur - saved).abs() - beta ** 2
                        merge = torch.where(abs_diff <= beta, quad_part, linear_part)
                        penalty += merge.sum()
    ...
    def compute_importances(
        ...
        for i, batch in enumerate(dataloader):
            ...
            x = tuple(_.to(device) for _ in list(x))
            y = tuple({k: v.to(device) for k, v in yy.items()} for yy in list(y))

            optimizer.zero_grad()

            detection_loss_dict = model(x, y)
            loss = sum(loss for loss in detection_loss_dict.values())
            loss.backward()
